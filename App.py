...
# ==============================================================================
# BULLET-PROOF SMART PARSING ENGINE (PURE AI + ACCURATE SMART FALLBACK)
# ==============================================================================
def smart_accurate_rule_parser(chunk_text, default_phase):
    """Accurate rule-based parsing that never hallucinates fake blocks."""
    ...
    for line in lines:
        ...
        if "PHASE 12" in l_up or "EME" in l_up:
            ...
            continue
        elif "PHASE 9 PRISM" in l_up or "9PRISM" in l_up or "9 PRISM" in l_up:
        ...
        # Check for Plot match (e.g. C-858 @ 485 lac, A-61 Demand -260, E 654)
...
def process_single_chunk_via_gemini(chunk_text, default_phase):
    ...
    prompt = f"""You are the Master DHA Lahore Real Estate AI Parser.
Extract EVERY valid property listing AND every Dealer Contact from the input text into a JSON array of objects.
...
RULES:
1. Block must be official (e.g. 'Block A', 'Block C', 'Block W', 'Zone 3 Commercial'). NEVER invent fake blocks like 'Block ARLA' or 'Block MAND'.
2. Extract exact Plot No (e.g. 'Plot 858', 'Plot 61', 'Plot 654').
3. Extract Size ('5 Marla', '10 Marla', '1 Kanal', '2 Kanal') and Demand ('485 Lac', '260 Lac', '2 Crore').
4. Attach dealer phone numbers to the listings.
...
[
  {{
    ...
    # If API fails or throttles, run guaranteed smart accurate parser
...
# 9. Main Clean Live Summary Dashboard
    ...
    if total_parsed_now > 0:
        if edit_summary_mode:
            st.info("💡 **Edit Mode Active:** Modify cells or delete rows.")
    ...
    raw_text = st.text_area(
        ...
        height=260,
        placeholder="Paste thousands of WhatsApp chat messages here...",
